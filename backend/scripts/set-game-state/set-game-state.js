...
export const setGameState = async () => {
  ...
  const gameState = process.argv.slice(2)[1];
  ...
  if (!gameState) {
    ...
    newGameState = JSON.parse(
      threadMessages.data[0].content[0].text.value
    );

    console.log("New game state: ", newGameState);
  } else {
    newGameState = JSON.parse(gameState);
  }
  ...
  await GameRoomService.updateGameRoom(gameId, {
    game_state: newEncryptedState,
  });
