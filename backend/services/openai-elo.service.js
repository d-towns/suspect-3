        ...
        const toolOutputs =
          data.required_action.submit_tool_outputs.tool_calls.map((toolCall) => {
            if (toolCall.function.name === "calculateEloChanges") {
                console.log("Calculating ELO changes for players:", toolCall.function);
              return {
                tool_call_id: toolCall.id,
                output: OpenAIEloService.calculateEloChanges(toolCall.function.arguments.players),
              };
            }
          });
...
Analyze the game thread thoroughly to assign appropriate ELO changes and badges.`,
            response_format: zodResponseFormat(this.LeaderboardSchema, 'leaderboard_results'),
            model: "gpt-4o-2024-08-06",
            tools: [{
                type: "function",
                function: {
                    name: "calculateEloChanges",
                    description: "Calculate ELO rating changes for players based on game results",
                    parameters: {
                        type: "object",
                        properties: {
                            players: {
...
static async calculateEloChanges(players) {
    ...
    const K = 32;
    const averageElo = players.reduce((acc, player) => acc + player.elo, 0) / players.length;
...
static async addPlayerEloToThread(threadId, players) {
  ...
  static async processGameThread(threadId, roomId) {
    ...
    try {
      ...
      stream = await this.client.beta.threads.runs.stream(threadId, {
      ...
      if (leaderboardData) {
        ...
        socketServer.emitToRoom(roomId, "leaderboard-stats", leaderboardData);
