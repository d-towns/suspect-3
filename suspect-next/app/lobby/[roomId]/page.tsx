...
import { ChatMessage, GameRoom, LobbyUser } from '@/lib/models';
import { CircleAlert } from 'lucide-react';
import { User } from '@supabase/supabase-js';
import { createClient } from '@/utils/supabase/client';
import { useSocketContext } from '@/lib/hooks/SocketContext/socket.context';
import { useToast } from '@/lib/hooks/ToastContext/toast.context';


type PlayersMap = Map<string, LobbyUser>;
...
//   const { addToast } = useToast();
  ...
  useEffect(() => {
    ...
    const setupListeners = () => {
      if (socket && isConnected) {
        ...
        socket.on('game-created', () => router.push(`/game/${roomId}`));
  ...
  }, [lobbyState.room, router, roomId]);

  const removePlayer = (player: LobbyUser) => {
  ...
  const addPlayer = (player: LobbyUser) => {
  ...
  const updatePlayerList = (newPlayers: LobbyUser[]) => {
    console.log('Updating player list:', newPlayers);
    setLobbyState(prevState => {
      const updatedPlayers = new Map<string, LobbyUser>();
  ...
  const handleSendMessage = () => {
    ...
    if (lobbyState.chat.inputMessage && user) {
      ...
      setLobbyState(prevState => ({
        ...prevState,
        chat: {
          messages: [...prevState.chat.messages, { userEmail: user.user_metadata.username, message: lobbyState.chat.inputMessage }],
          inputMessage: '',
        },
      }));
    }
  ...
  const handleReadyClick = () => {
    ...
    setLobbyState(prevState => {
      const updatedPlayers = new Map(prevState.players);
      if (user) {
        const currentPlayer = updatedPlayers.get(user.user_metadata.username);
        if (currentPlayer) {
          updatedPlayers.set(user.user_metadata.username, { ...currentPlayer, isReady: newReadyStatus });
        }
      }
      const allPlayersReady = Array.from(updatedPlayers.values()).every(player => player.isReady);
      return {
            ...
            <Box mt="4">
              {Array.from(lobbyState.players.values()).map((player, index) => (
                ...
                <Flex key={player.username || player.email} align="center" gap="2" mt="4">
                  ...
                  {player.username === user?.user_metadata.username && <Badge color="blue">You</Badge>}
                  {player?.id === lobbyState.room?.host_id && <Badge color="orange">Host</Badge>}
                  {lobbyState.gameStatus.userIsHost && player.username !== user?.user_metadata.username && (
